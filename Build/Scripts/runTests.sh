...
read -r -d '' HELP <<EOF
jobrouter_process test runner. Execute unit test suite and some other details.
Also used by GitHub Actions for test execution.

Successfully tested with docker version 20.10.10-ce and docker-compose 1.25.0.
...
while getopts ":s:d:p:e:t:xy:hv" OPT; do
    case ${OPT} in
        s)
            TEST_SUITE=${OPTARG}
            ;;
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="web/typo3conf/ext/jobrouter_process/${1}"
fi
